...
def upgrade() -> None:
    with op.batch_alter_table("chunks", schema=None) as batch_op:
        batch_op.create_foreign_key("fk_chunks_document_id", "documents", ["document_id"], ["id"])
    ...
    with op.batch_alter_table("claims", schema=None) as batch_op:
        batch_op.create_foreign_key("fk_claims_chunk_id", "chunks", ["chunk_id"], ["id"])
        batch_op.create_foreign_key("fk_claims_document_id", "documents", ["document_id"], ["id"])

    with op.batch_alter_table("claim_entities", schema=None) as batch_op:
        batch_op.create_foreign_key("fk_claim_entities_claim_id", "claims", ["claim_id"], ["id"])
        batch_op.create_foreign_key(
    ...
    with op.batch_alter_table("relationships", schema=None) as batch_op:
        ...
        batch_op.create_foreign_key("fk_relationships_claim_id", "claims", ["claim_id"], ["id"])
