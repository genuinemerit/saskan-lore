# tests/unit/r5_retrieval/conftest.py
"""
R5 retrieval and answering test configuration.

Mocks llama_cpp in sys.modules before any test module is imported so that
inference.py (imported by answering.py) can be loaded without a real GGUF
model file on disk.
...
# against a real file. Individual tests patch complete() for response control.
